...
% called by "prepare.m"
%
% use to test the flank above the preferred direction


function [unit] = tbtv_final_above(unit,use_fraction)
...
numtrials = size(unit.above.evel,1);
tlen = size(unit.binaryspikes,2);

unit.above.filterbank.var = single(zeros(tlen,numdraws));
unit.above.filterbank.randvar = single(zeros(tlen,numdraws));
unit.above.filterbank.best = single(zeros(tlen,numdraws));

unit.above.test.n = single(zeros(1,numdraws));
unit.above.test.corr = single(zeros(1,numdraws));
unit.above.test.timecorr = single(zeros(numdraws,tlen));
unit.above.test.mse = single(zeros(1,numdraws));
unit.above.test.timemse = single(zeros(numdraws,tlen));

unit.above.test.shufcorr = single(zeros(1,numdraws));
unit.above.test.shuftimecorr = single(zeros(numdraws,tlen));
unit.above.test.shufmsedist = single(zeros(1,numdraws));
unit.above.test.shuftimemse = single(zeros(numdraws,tlen));

unit.above.cross.corr = single(zeros(1,numdraws));
unit.above.cross.timecorr = single(zeros(numdraws,tlen));
unit.above.cross.bestcorr = single(zeros(1,numdraws));
unit.above.cross.shufcorr = single(zeros(1,numdraws));

unit.above.bytrial = nan(numdraws,numtrials);
%%%%%%%%%%%%%%%%%%%%%%
% general trial info %
%%%%%%%%%%%%%%%%%%%%%%

spikes = unit.above.binaryspikes;
eye = unit.above.evel;
...
% save the filters
unit.above.filterbank.var = single(squeeze(varsta));
unit.above.filterbank.randvar = single(squeeze(shufvarsta));
unit.above.filterbank.best = single(squeeze(beststa));
...
for draw = 1:numdraws
  ...
  unit.above.test.n(draw) = single(length(predicted_var(:)));

  % analyze the relationship
  % (correlation coefficient and mean square error alone and in time)
  unit.above.test.corr(draw) = single(corr2(predicted_var(:),actual_var(:)));
  unit.above.test.mse(draw) = single(mean(mean((actual_var - predicted_var).^2,2)));
  unit.above.test.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  unit.above.test.timemse(draw,:) = single(mean((actual_var - predicted_var).^2,2));
  ...
  unit.above.cross.corr(draw) = single(corr2(predicted_full(:),actual_full(:)));
  unit.above.cross.bestcorr(draw) = single(corr2(best_predicted_full(:),actual_full(:)));
  unit.above.cross.shufcorr(draw) = single(corr2(shuf_predicted_full(:),actual_full(:)));
  unit.above.cross.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  ...
  unit.above.test.shufcorr(draw) = single(corr2(shuffle_predicted_var(:),actual_var(:)));
  unit.above.test.shufmsedist(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  unit.above.test.shuftimecorr(draw,:) = single(mycorr(shuffle_predicted_var',actual_var'));
  unit.above.test.shuftimemse(draw,:) = single(mean((actual_var - shuffle_predicted_var).^2,2));
  unit.above.test.shufmse(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  %end

  % keep track of how we do for each trial
  unit.above.bytrial(draw,perm(draw,testindex)) = mycorr(predicted_var,actual_var);
...
if makefig
 ...
 % title([unit.dir(end-7:end),'.',num2str(unit.localid)]);
  ...
  subplot(3,2,3)
  hold on
  plot(-filtlen:filtlen,flipud(squeeze(unit.above.filterbank.randvar(:,:))),'color',[.6 .6 .6])
  plot(-filtlen:filtlen,flipud(squeeze(unit.above.filterbank.var(:,:))),'color',pretty('i'))
  set(gca,'tickdir','out',...
  ...
  % plot overall performance
  crossvals = unit.above.cross.corr(:);
  bestcross = unit.above.cross.bestcorr(:);
  shufcross = unit.above.cross.shufcorr(:);
  ...
  subplot(3,2,6)
  hold on
  plot(-200:triallen,squeeze(unit.above.test.timecorr(:,:)),'color',pretty('i'))
  shufvals = squeeze(unit.above.test.shuftimecorr(:,:,:));
