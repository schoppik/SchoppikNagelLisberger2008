...
% called by "prepare.m"
%
% use to test the flank below the preferred direction


function [unit] = tbtv_final_below(unit,use_fraction)
...
numtrials = size(unit.below.evel,1);
tlen = size(unit.binaryspikes,2);

unit.below.filterbank.var = single(zeros(tlen,numdraws));
unit.below.filterbank.randvar = single(zeros(tlen,numdraws));
unit.below.filterbank.best = single(zeros(tlen,numdraws));

unit.below.test.n = single(zeros(1,numdraws));
unit.below.test.corr = single(zeros(1,numdraws));
unit.below.test.timecorr = single(zeros(numdraws,tlen));
unit.below.test.mse = single(zeros(1,numdraws));
unit.below.test.timemse = single(zeros(numdraws,tlen));

unit.below.test.shufcorr = single(zeros(1,numdraws));
unit.below.test.shuftimecorr = single(zeros(numdraws,tlen));
unit.below.test.shufmsedist = single(zeros(1,numdraws));
unit.below.test.shuftimemse = single(zeros(numdraws,tlen));

unit.below.cross.corr = single(zeros(1,numdraws));
unit.below.cross.timecorr = single(zeros(numdraws,tlen));
unit.below.cross.bestcorr = single(zeros(1,numdraws));
unit.below.cross.shufcorr = single(zeros(1,numdraws));

unit.below.bytrial = nan(numdraws,numtrials);
%%%%%%%%%%%%%%%%%%%%%%
% general trial info %
%%%%%%%%%%%%%%%%%%%%%%

spikes = unit.below.binaryspikes;
eye = unit.below.evel;
...
% save the filters
unit.below.filterbank.var = single(squeeze(varsta));
unit.below.filterbank.randvar = single(squeeze(shufvarsta));
unit.below.filterbank.best = single(squeeze(beststa));
...
for draw = 1:numdraws
  ...
  unit.below.test.n(draw) = single(length(predicted_var(:)));

  % analyze the relationship
  % (correlation coefficient and mean square error alone and in time)
  unit.below.test.corr(draw) = single(corr2(predicted_var(:),actual_var(:)));
  unit.below.test.mse(draw) = single(mean(mean((actual_var - predicted_var).^2,2)));
  unit.below.test.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  unit.below.test.timemse(draw,:) = single(mean((actual_var - predicted_var).^2,2));
  ...
  unit.below.cross.corr(draw) = single(corr2(predicted_full(:),actual_full(:)));
  unit.below.cross.bestcorr(draw) = single(corr2(best_predicted_full(:),actual_full(:)));
  unit.below.cross.shufcorr(draw) = single(corr2(shuf_predicted_full(:),actual_full(:)));
  unit.below.cross.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  ...
  unit.below.test.shufcorr(draw) = single(corr2(shuffle_predicted_var(:),actual_var(:)));
  unit.below.test.shufmsedist(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  unit.below.test.shuftimecorr(draw,:) = single(mycorr(shuffle_predicted_var',actual_var'));
  unit.below.test.shuftimemse(draw,:) = single(mean((actual_var - shuffle_predicted_var).^2,2));
  unit.below.test.shufmse(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  %end

  % keep track of how we do for each trial
  unit.below.bytrial(draw,perm(draw,testindex)) = mycorr(predicted_var,actual_var);
...
if makefig
 ...
 % title([unit.dir(end-7:end),'.',num2str(unit.localid)]);
  ...
  subplot(3,2,3)
  hold on
  plot(-filtlen:filtlen,flipud(squeeze(unit.below.filterbank.randvar(:,:))),'color',[.6 .6 .6])
  plot(-filtlen:filtlen,flipud(squeeze(unit.below.filterbank.var(:,:))),'color',pretty('i'))
  set(gca,'tickdir','out',...
  ...
  % plot overall performance
  crossvals = unit.below.cross.corr(:);
  bestcross = unit.below.cross.bestcorr(:);
  shufcross = unit.below.cross.shufcorr(:);
  ...
  subplot(3,2,6)
  hold on
  plot(-200:triallen,squeeze(unit.below.test.timecorr(:,:)),'color',pretty('i'))
  shufvals = squeeze(unit.below.test.shuftimecorr(:,:,:));
