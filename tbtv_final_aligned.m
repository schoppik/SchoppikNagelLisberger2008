...
%
% called by "prepare.m"
% use for data aligned by "minimize_variance.m"
...
makefig = 0; % make a figure for each unit
...
% align the trials
% expects "minimize_variance" to have been run, resulting in the unit.shift
% parameter


start = max(unit.shift);
if ~start % was it zero?
  start = 1;
end
...
for trial = 1:numtrials
  eye_on_trial = circshift(unit.evel(trial,:),[0 unit.shift(trial)]);
  eye(trial,:) = eye_on_trial(start:end-stop);
  
  spikes_on_trial = circshift(unit.binaryspikes(trial,:),[0 unit.shift(trial)]);
  spikes(trial,:) = spikes_on_trial(start:end-stop);
end

tlen = size(spikes,2);

unit.aligned.filterbank.var = single(zeros(tlen,numdraws));
unit.aligned.filterbank.randvar = single(zeros(tlen,numdraws));
unit.aligned.filterbank.best = single(zeros(tlen,numdraws));

unit.aligned.test.n = single(zeros(1,numdraws));
unit.aligned.test.corr = single(zeros(1,numdraws));
unit.aligned.test.timecorr = single(zeros(numdraws,tlen));
unit.aligned.test.mse = single(zeros(1,numdraws));
unit.aligned.test.timemse = single(zeros(numdraws,tlen));

unit.aligned.test.shufcorr = single(zeros(1,numdraws));
unit.aligned.test.shuftimecorr = single(zeros(numdraws,tlen));
unit.aligned.test.shufmsedist = single(zeros(1,numdraws));
unit.aligned.test.shuftimemse = single(zeros(numdraws,tlen));

unit.aligned.cross.corr = single(zeros(1,numdraws));
unit.aligned.cross.timecorr = single(zeros(numdraws,tlen));
unit.aligned.cross.bestcorr = single(zeros(1,numdraws));
unit.aligned.cross.shufcorr = single(zeros(1,numdraws));

unit.aligned.bytrial = nan(numdraws,numtrials);
%%%%%%%%%%%%%%%%%%%%%%
% general trial info %
%%%%%%%%%%%%%%%%%%%%%%
...
% save the filters
unit.aligned.filterbank.var = single(squeeze(varsta));
unit.aligned.filterbank.randvar = single(squeeze(shufvarsta));
unit.aligned.filterbank.best = single(squeeze(beststa));
...
for draw = 1:numdraws
  ...
  unit.aligned.test.n(draw) = single(length(predicted_var(:)));

  % analyze the relationship
  % (correlation coefficient and mean square error alone and in time)
  unit.aligned.test.corr(draw) = single(corr2(predicted_var(:),actual_var(:)));
  unit.aligned.test.mse(draw) = single(mean(mean((actual_var - predicted_var).^2,2)));
  unit.aligned.test.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  unit.aligned.test.timemse(draw,:) = single(mean((actual_var - predicted_var).^2,2));
  ...
  unit.aligned.cross.corr(draw) = single(corr2(predicted_full(:),actual_full(:)));
  unit.aligned.cross.bestcorr(draw) = single(corr2(best_predicted_full(:),actual_full(:)));
  unit.aligned.cross.shufcorr(draw) = single(corr2(shuf_predicted_full(:),actual_full(:)));
  unit.aligned.cross.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  ...
  unit.aligned.test.shufcorr(draw) = single(corr2(shuffle_predicted_var(:),actual_var(:)));
  unit.aligned.test.shufmsedist(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  unit.aligned.test.shuftimecorr(draw,:) = single(mycorr(shuffle_predicted_var',actual_var'));
  unit.aligned.test.shuftimemse(draw,:) = single(mean((actual_var - shuffle_predicted_var).^2,2));
  unit.aligned.test.shufmse(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  %end

  % keep track of how we do for each trial
  unit.aligned.bytrial(draw,perm(draw,testindex)) = mycorr(predicted_var,actual_var);
...
if makefig
 ...
 % title([unit.dir(end-7:end),'.',num2str(unit.localid)]);
  ...
  subplot(3,2,3)
  hold on
  plot(-filtlen:filtlen,flipud(squeeze(unit.aligned.filterbank.randvar(:,:))),'color',[.6 .6 .6])
  plot(-filtlen:filtlen,flipud(squeeze(unit.aligned.filterbank.var(:,:))),'color',pretty('i'))
  set(gca,'tickdir','out',...
  ...
  subplot(3,2,6)
  hold on
  plot(-200:triallen,squeeze(unit.aligned.test.timecorr(:,:)),'color',pretty('i'))
  shufvals = squeeze(unit.aligned.test.shuftimecorr(:,:,:));
