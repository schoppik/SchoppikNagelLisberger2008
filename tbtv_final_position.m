...
% called by "prepare.m"
%
% use to run on the position trace

function [unit] = tbtv_final_position(unit,use_fraction)
...
unit.position.filterbank.var = single(zeros(tlen,numdraws));
unit.position.filterbank.randvar = single(zeros(tlen,numdraws));
unit.position.filterbank.best = single(zeros(tlen,numdraws));

unit.position.test.n = single(zeros(1,numdraws));
unit.position.test.corr = single(zeros(1,numdraws));
unit.position.test.timecorr = single(zeros(numdraws,tlen));
unit.position.test.mse = single(zeros(1,numdraws));
unit.position.test.timemse = single(zeros(numdraws,tlen));

unit.position.test.shufcorr = single(zeros(1,numdraws));
unit.position.test.shuftimecorr = single(zeros(numdraws,tlen));
unit.position.test.shufmsedist = single(zeros(1,numdraws));
unit.position.test.shuftimemse = single(zeros(numdraws,tlen));

unit.position.cross.corr = single(zeros(1,numdraws));
unit.position.cross.timecorr = single(zeros(numdraws,tlen));
unit.position.cross.bestcorr = single(zeros(1,numdraws));
unit.position.cross.shufcorr = single(zeros(1,numdraws));

unit.position.bytrial = nan(numdraws,numtrials);
%%%%%%%%%%%%%%%%%%%%%%
% general trial info %
%%%%%%%%%%%%%%%%%%%%%%
...
% save the filters
unit.position.filterbank.var = single(squeeze(varsta));
unit.position.filterbank.randvar = single(squeeze(shufvarsta));
unit.position.filterbank.best = single(squeeze(beststa));
...
for draw = 1:numdraws
  ...
  unit.position.test.n(draw) = single(length(predicted_var(:)));

  % analyze the relationship
  % (correlation coefficient and mean square error alone and in time)
  unit.position.test.corr(draw) = single(corr2(predicted_var(:),actual_var(:)));
  unit.position.test.mse(draw) = single(mean(mean((actual_var - predicted_var).^2,2)));
  unit.position.test.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  unit.position.test.timemse(draw,:) = single(mean((actual_var - predicted_var).^2,2));
  ...
  unit.position.cross.corr(draw) = single(corr2(predicted_full(:),actual_full(:)));
  unit.position.cross.bestcorr(draw) = single(corr2(best_predicted_full(:),actual_full(:)));
  unit.position.cross.shufcorr(draw) = single(corr2(shuf_predicted_full(:),actual_full(:)));
  unit.position.cross.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  ...
  unit.position.test.shufcorr(draw) = single(corr2(shuffle_predicted_var(:),actual_var(:)));
  unit.position.test.shufmsedist(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  unit.position.test.shuftimecorr(draw,:) = single(mycorr(shuffle_predicted_var',actual_var'));
  unit.position.test.shuftimemse(draw,:) = single(mean((actual_var - shuffle_predicted_var).^2,2));
  unit.position.test.shufmse(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  %end

  % keep track of how we do for each trial
  unit.position.bytrial(draw,perm(draw,testindex)) = mycorr(predicted_var,actual_var);
...
if makefig
 ...
 % title([unit.dir(end-7:end),'.',num2str(unit.localid)]);
  ...
  subplot(3,2,3)
  hold on
  plot(-filtlen:filtlen,flipud(squeeze(unit.position.filterbank.randvar(:,:))),'color',[.6 .6 .6])
  plot(-filtlen:filtlen,flipud(squeeze(unit.position.filterbank.var(:,:))),'color',pretty('i'))
  set(gca,'tickdir','out',...
  ...
  % plot overall performance
  crossvals = unit.position.cross.corr(:);
  bestcross = unit.position.cross.bestcorr(:);
  shufcross = unit.position.cross.shufcorr(:);
  ...
  subplot(3,2,6)
  hold on
  plot(-200:triallen,squeeze(unit.position.test.timecorr(:,:)),'color',pretty('i'))
  shufvals = squeeze(unit.position.test.shuftimecorr(:,:,:));
