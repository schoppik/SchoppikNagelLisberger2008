...
%
% called by "prepare.m"
% use to control for the consequences of shorter trials on aligned data


function [unit] = tbtv_final_short(unit,use_fraction)
...
for trial = 1:numtrials
  eye_on_trial = unit.evel(trial,:);
  eye(trial,:) = eye_on_trial(start:end-stop);
  
  spikes_on_trial = unit.binaryspikes(trial,:);
  spikes(trial,:) = spikes_on_trial(start:end-stop);
end

tlen = size(spikes,2);

unit.short.filterbank.var = single(zeros(tlen,numdraws));
unit.short.filterbank.randvar = single(zeros(tlen,numdraws));
unit.short.filterbank.best = single(zeros(tlen,numdraws));

unit.short.test.n = single(zeros(1,numdraws));
unit.short.test.corr = single(zeros(1,numdraws));
unit.short.test.timecorr = single(zeros(numdraws,tlen));
unit.short.test.mse = single(zeros(1,numdraws));
unit.short.test.timemse = single(zeros(numdraws,tlen));

unit.short.test.shufcorr = single(zeros(1,numdraws));
unit.short.test.shuftimecorr = single(zeros(numdraws,tlen));
unit.short.test.shufmsedist = single(zeros(1,numdraws));
unit.short.test.shuftimemse = single(zeros(numdraws,tlen));

unit.short.cross.corr = single(zeros(1,numdraws));
unit.short.cross.timecorr = single(zeros(numdraws,tlen));
unit.short.cross.bestcorr = single(zeros(1,numdraws));
unit.short.cross.shufcorr = single(zeros(1,numdraws));

unit.short.bytrial = nan(numdraws,numtrials);
%%%%%%%%%%%%%%%%%%%%%%
% general trial info %
%%%%%%%%%%%%%%%%%%%%%%
...
% save the filters
unit.short.filterbank.var = single(squeeze(varsta));
unit.short.filterbank.randvar = single(squeeze(shufvarsta));
unit.short.filterbank.best = single(squeeze(beststa));
...
for draw = 1:numdraws
  ...
  unit.short.test.n(draw) = single(length(predicted_var(:)));

  % analyze the relationship
  % (correlation coefficient and mean square error alone and in time)
  unit.short.test.corr(draw) = single(corr2(predicted_var(:),actual_var(:)));
  unit.short.test.mse(draw) = single(mean(mean((actual_var - predicted_var).^2,2)));
  unit.short.test.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  unit.short.test.timemse(draw,:) = single(mean((actual_var - predicted_var).^2,2));
  ...
  unit.short.cross.corr(draw) = single(corr2(predicted_full(:),actual_full(:)));
  unit.short.cross.bestcorr(draw) = single(corr2(best_predicted_full(:),actual_full(:)));
  unit.short.cross.shufcorr(draw) = single(corr2(shuf_predicted_full(:),actual_full(:)));
  unit.short.cross.timecorr(draw,:) = single(mycorr(predicted_var',actual_var'));
  ...
  unit.short.test.shufcorr(draw) = single(corr2(shuffle_predicted_var(:),actual_var(:)));
  unit.short.test.shufmsedist(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  unit.short.test.shuftimecorr(draw,:) = single(mycorr(shuffle_predicted_var',actual_var'));
  unit.short.test.shuftimemse(draw,:) = single(mean((actual_var - shuffle_predicted_var).^2,2));
  unit.short.test.shufmse(draw) = single(mean(mean((actual_var - shuffle_predicted_var).^2,2)));
  %end

  % keep track of how we do for each trial
  unit.short.bytrial(draw,perm(draw,testindex)) = mycorr(predicted_var,actual_var);
...
if makefig
 ...
 % title([unit.dir(end-7:end),'.',num2str(unit.localid)]);
  ...
  subplot(3,2,3)
  hold on
  plot(-filtlen:filtlen,flipud(squeeze(unit.short.filterbank.randvar(:,:))),'color',[.6 .6 .6])
  plot(-filtlen:filtlen,flipud(squeeze(unit.short.filterbank.var(:,:))),'color',pretty('i'))
  set(gca,'tickdir','out',...
  ...
  subplot(3,2,6)
  hold on
  plot(-200:triallen,squeeze(unit.short.test.timecorr(:,:)),'color',pretty('i'))
  shufvals = squeeze(unit.short.test.shuftimecorr(:,:,:));
